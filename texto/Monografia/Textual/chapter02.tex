...
Para maior aprofundamento nas técnicas e mecanismos de aquisição das métricas apresentadas nesta seção, a comunidade desenvolvedora do \foreign{kernel} do Linux fornece uma extensa documentação \citep{linuxkernel2025} para estudo. {\color{red}{De forma geral, estas métricas são obtidas a partir da leitura de arquivos do sistema de arquivos (\foreign{filesystem}) do Linux, localizados no diretório \verb|/proc|, que expõem informações sobre o estado atual do sistema.}
}
...
\begin{itemize}
\item \textbf{Espaço livre}: quantidade de espaço livre em disco, \textcolor{red}{em bytes. Quando o espaço livre está proporcionalmente próximo} de 0, pode causar degradação de desempenho, corrupção de dados, congelamentos e \foreign{crashes};
...
Algumas de suas características são a execução de \foreign{plugins} Go, facilitando extensão e manutenção, concorrência melhorada, armazenamento persistente, suporte nativo a monitoramento de tecnologias como Docker, e tanto verificações passivas quanto ativas suportam intervalos programados e flexíveis.
...
O trabalho de Antônio José Alves Neto \citep{alvesneto2023} tem por objetivo o desenvolvimento e avaliação de desempenho de um cluster em aplicações big data. Utilizando Raspberry Pis para implementação do hardware do cluster, o autor alinha-se com a ideia de que a utilização de Raspberry Pis é uma boa solução na obtenção de um cluster big data de baixo custo. Além disso, utilizando-se de ferramentas Zabbix para coleta e Grafana para visualização, o autor pôde observar a utilização de recursos do cluster, tais como: uso de CPU, memória usada,
...
% Bhanuprakash Madupati \citep{madupati2023} em seu trabalho sobre observabilidade em arquiteturas em microsserviços, propõe a utilização do Prometheus para a coleta de métricas de saturação, Grafana para visualização e Jaeger (uma ferramenta de código aberto desenvolvida pela Uber) para rastreamento distribuído. O autor demonstra como essas ferramentas podem ser integradas para fornecer uma visão abrangente do desempenho de aplicações em microsserviços, permitindo a identificação de gargalos e a otimização de recursos. Além disso, o autor menciona a importância da automatização de instruções para centralização,  consistência e maior facilidade de manutenção do sistema, práticas em conformidade com príncipios de IaC.
...
% Praveen Chaitanya Jakku \citep{jakku2025} também aborda o papel fundamental do monitoramento em soluções de problemas e otimização na cultura DevOps (\foreign{Development and Operations}\abbrev{DevOps}{\foreign{Development and Operations}}). Em seu trabalho, ele apresenta conceitos e práticas de DevOps onde, dentre outros temas, discutem-se:
% \begin{itemize}
%     \item Métricas de uso de CPU, memória, I/O de disco e rede como indicativos de estabilidade do sistema;
%     \item Monitoramento da alocação e uso de recursos de contêineres;
%     \item Alertas e notificações para indícios de degradação de desempenho;
%     \item Utilização de ferramentas como Prometheus e Grafana no monitoramento;
%     \item Apresenta casos de estudo onde grandes plataformas como Uber e LinkedIn utilizam o Prometheus (aliado de outras ferramentas) para monitoramento de suas aplicações, otimizando o desempenho e a confiabilidade de seus serviços.
% \end{itemize}

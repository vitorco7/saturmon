...
{\color{red}
As métricas apresentadas neste trabalho são extraídas por meio da leitura de arquivos do Linux. Especificamente para métricas de CPU, memória e I/O de disco dos dispositivos virtuais, a coleta ocorre a partir de arquivos localizados no diretório \verb|/sys/fs/cgroup|.

Durante o desenvolvimento multi-máquina, especialmente na etapa de configuração manual dos parâmetros do \verb|inputs.exec| do Telegraf, foram identificadas inconsistências e erros na leitura das métricas entre as implementações feitas no \foreign{notebook} e no \foreign{desktop}. Como apresentado em \ref{section:Infraestrutura}, o fato do \foreign{notebook} operar com o Ubuntu 20 implica que, por padrão, o mesmo apresenta a versão 1 do \foreign{cgroups}, enquanto o \foreign{desktop}, com Ubuntu 24, utiliza a versão 2. Essa disparidade nas versões do \foreign{cgroups} resultou em diferenças estruturais nos arquivos de leitura das métricas, ocasionando no impedimento de um arquivo de configuração único e padronizado, devido à falhas de coleta. Esse problema foi solucionado com a atualização do \foreign{cgroups} do \foreign{notebook} para a versão 2, harmonizando as estruturas dos arquivos e, consequentemente, permitindo a padronização das configurações de coleta.
...
\item \textbf{Relevância}: As métricas escolhidas devem fornecer informações essenciais para o monitoramento da saturação do sistema, permitindo tanto a detecção imediata de problemas e a adoção de medidas corretivas quanto o auxílio de análises mais aprofundadas.
...
\multirow{4}{*}{CPU} & usage\_idle      & --           \\ \cline{2-3}
                     & usage\_iowait    & --           \\ \cline{2-3}
                     & usage\_system    & system\_usec  \\ \cline{2-3}
                     & usage\_user      & user\_usec    \\ \hline
\multirow{4}{*}{Memória} & used     & memory.current                      \\ \cline{2-3}
                         & total       & memory.max                          \\ \cline{2-3}
                         & swap\_free & memory.swap.max                     \\ \cline{2-3}
                         & swap\_total  & memory.swap.max-memory.swap.current \\ \hline
\multirow{6}{*}{Disco} & free          & --      \\ \cline{2-3}
                      & used         & --      \\ \cline{2-3}
                      & total         & --      \\ \cline{2-3}
                      & inodes\_free  & --      \\ \cline{2-3}
                      & read\_bytes   & rbytes  \\ \cline{2-3}
                      & write\_bytes  & wbytes  \\ \cline{2-3}
                      & io\_util      & --      \\ \hline
\multirow{4}{*}{Processos} & running  & running  \\ \cline{2-3}
                           & sleeping & sleeping \\ \cline{2-3}
                           & total    & total    \\ \cline{2-3}
                           & zombies  & zombies  \\ \hline
\multirow{8}{*}{Rede} & bytes\_recv   & Bytes Received   \\ \cline{2-3}
                      & bytes\_sent       & Bytes Sent       \\ \cline{2-3}
                      & packets\_recv & Packets Received \\ \cline{2-3}
                      & packets\_sent     & Packets Sent     \\ \cline{2-3}
                      & drop\_in          & Drop In          \\ \cline{2-3}
                      & drop\_out         & Drop Out         \\ \cline{2-3}
                      & err\_in         & Error In         \\ \cline{2-3}
                      & err\_out        & Error Out        \\ \hline
\end{tabular}
\end{table}
